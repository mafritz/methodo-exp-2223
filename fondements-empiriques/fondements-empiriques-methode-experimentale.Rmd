...
```{r idealized-experiment, fig.height=3, fig.width=5, fig.cap="Données simulées sur une potentielle expérience avec un plan inter-sujets comprenant 3 modalités de la VI et une VD. Les barres représentent des intervalles de confiance à 95%."}
sans <- tibble(
  condition = "Texte",
...
Pour corroborer les hypothèses il serait néanmoins nécessaire un test statistique, dont les détails dépendent de l'approche statistique envisagée. Comme indiqué plus haut dans la partie sur le protocole expérimental, il est encore une fois de plus en plus de bonne pratique de fournir les détails précis des analyses à mener dans le pré-enregistrement de l'expérience. Par exemple, les chercheurs peuvent simuler des données compatibles avec leurs hypothèses et fournir en même temps le code informatique (e.g. un script dans le langage de programmation R) qui défini en amont les détails du modèle statistique qui sera utilisé.
