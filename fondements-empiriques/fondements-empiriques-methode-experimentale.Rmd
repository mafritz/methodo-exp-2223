...
## Hypothèses théoriques/causales et modèles structurels causaux
...
À ce moment, il faut tester si ce raisonnement imagé et idéalisé peut se produire dans la réalité. Pour ce faire, les chercheurs créent un *micro-monde* dans lequel produire les **conditions nécessaires et suffisantes** à déclencher d'abord et mesurer ensuite l'effet de l'intervention sur le phénomène d'intérêt. Le *micro-monde* vise à trouver un équilibre entre la représentativité de la *réalité* d'une parte, et la réduction de la complexité nécessaire pour pouvoir isoler l'effet de l'intervention d'autres effets potentiels.

![Le micro-monde vise à produire les conditions nécessaires et suffisantes aux objectifs de l'expérience.](images/macro-vs-micro-monde.png){width="70%"}

C'est au niveau du micro-monde qui sont générées, récoltées et analysées les données empiriques. Le micro-monde se compose des choix épistémologiques et méthodologiques des chercheurs qui s'articulent principalement dans les éléments suivants:

1.  l'opéralisation des variables concernées;

2.  le design expérimental;

3.  les entités observées, c'est-à-dire les participant-es dans la plupart des cas en technologie éducative;

4.  le matériel expérimental;

5.  le protocole ou la procédure expérimentale;

6.  l'analyse des données à mener pour déterminer la présence, la direction, la magnitude et l'incertitude autour de l'effet envisagé.
...
-   **Bloquer/neutraliser une variable en amont**\
...
Le tableau suivant montre les premières 10 participants.
...
Par conséquent, les participantes ont la tendance à privilégier naturellement l'intervention $X_1$ comparé à l'intervention $X_0$. À ce point, on peut simuler l'attribution de chaque participante à une modalité de la VI selon deux mécanismes :

-   Dans le premier mécanisme, les participant-es choisissent volontairement si *s'auto-attribuer* à la modalité $X_0$ ou $X_1$ selon leur propre prédisposition. En d'autres termes, si leur intérêt pour $X_0$ est majeur ou égale à l'intérêt pour $X_1$, alors ils sont attribué à $X_0$. Si par contre leur intérêt est majeur pour $X_1$ comparé à $X_0$, alors ils sont attribués à $X_1$.

-   Dans le deuxième mécanisme, chaque participant à exactement 50% de chances d'être attribué à $X_0$ ou $X_1$ indépendamment de sa prédisposition pour l'une ou pour l'autre modalité. Il s'agit en d'autres termes d'une attribution totalement randomisée.

Le tableau suivant montre les moyennes et l'écart type des variables âge, interest_X0 et interest_X1 pour l'auto-attribution préférentielle des participant-es. On peut noter comme déjà il y a plus de participant-es qui ont choisi l'intervention $X_1$ et que la moyenne d'intérêt pour cette modalité est plus élevée par rapport à $X_0$.
...
Cet autre tableau montre les même résultats, mais lorsque l'attribution est faite avec le deuxième mécanisme totalement aléatoire. On peut remarquer que tous les valeurs des variables sont plus proches les uns aux autres, notamment au niveau de la répartition de l'intérêt prédisposé pour $X_0$ et $X_1$.
...
Randomisation bloquée avec ou sans remplacement. Dans une randomisation bloquée, les entités/participant-es sont d'abord séparée par rapport à une caractéristique/variable spécifique, et seulement une fois cette séparation effectuée, l'attribution aux modalités.

### Design inter-sujet, intra-sujet ou mixte

### Design expérimental à mesure répetée
